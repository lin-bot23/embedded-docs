...
| `圖片種子` | 用於圖像生成階段的種子（預設：42）。 | INT | 否 | 0 to 2147483647 |
| `模型種子` | 用於模型生成階段的種子（預設：42）。 | INT | 否 | 0 to 2147483647 |
| `紋理種子` | 用於紋理生成階段的種子（預設：42）。 | INT | 否 | 0 to 2147483647 |
| `紋理品質` | 生成紋理的品質。detailed = HD 紋理，extreme = 8K Ultra 紋理（預設：standard）。 | COMBO | 否 | "standard"<br>"detailed"<br>"extreme" |
| `面數限制` | 最大面數。-1 讓 Tripo 自適應選擇（v3.x standard 約 1.4M 面，detailed 約 2M 面）。Tripo 會默默限制：v2.5 為 500,000，四邊形網格為 150,000。（預設：-1） | INT | 否 | -1 to 2000000 |
