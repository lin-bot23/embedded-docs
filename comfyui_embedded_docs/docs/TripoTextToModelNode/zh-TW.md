...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `提示詞` | 欲生成 3D 模型的文字描述（多行）。此參數為必填，且不能為空。 | STRING | 是 | - |
| `負向提示詞` | 要在生成的模型中避免出現之內容的文字描述（多行）。最多 255 個字元。僅在非空時才會傳送至 API。 | STRING | 否 | 最多 255 個字元 |
| `模型版本` | 用於生成的 Tripo 模型版本（預設值：v3.1-20260211）。 | COMBO | 否 | 有多個選項可供選擇 |
| `風格` | 套用到生成模型的樣式（預設值：None）。Tripo 已不再支援且會忽略此參數；僅為舊版工作流程保留。 | COMBO | 否 | 有多個選項可供選擇 |
| `紋理` | 是否生成紋理貼圖。關閉時只會回傳不帶紋理的幾何模型，並忽略 `pbr`（預設值：True）。 | BOOLEAN | 否 | true / false |
| `PBR材質` | 是否生成 PBR 材質貼圖（基底色彩、金屬度、粗糙度、法線）。需要 `texture`；當 `texture` 關閉時會強制關閉（預設值：True）。 | BOOLEAN | 否 | true / false |
| `圖片種子` | 用於影像生成階段的種子（預設值：42）。 | INT | 否 | 0 至 2147483647 |
| `模型種子` | 用於模型生成階段的種子（預設值：42）。 | INT | 否 | 0 至 2147483647 |
| `紋理種子` | 用於紋理生成階段的種子（預設值：42）。 | INT | 否 | 0 至 2147483647 |
| `紋理品質` | 生成紋理的品質。detailed = HD 紋理，extreme = 8K Ultra 紋理（預設值：standard）。 | COMBO | 否 | "standard"<br>"detailed"<br>"extreme" |
| `面數限制` | 最大面數。設為 -1 時由 Tripo 自適應選擇（在 v3.x 標準模式下約 1.4M 面，detailed 模式下約 2M 面）。Tripo 會靜默限制上限：v2.5 為 500,000，四邊形網格為 150,000。（預設值：-1） | INT | 否 | -1 至 2000000 |
| `四邊形` | 四邊形網格輸出。Tripo 以 FBX 格式提供四邊形網格，因此結果會出現在 FBX 輸出，而 GLB 輸出保持為空。（預設值：False） | BOOLEAN | 否 | true / false |
| `幾何品質` | 生成幾何的品質（預設值：standard）。 | COMBO | 否 | "standard"<br>"detailed" |
...
| 輸出名 | 描述 | 資料類型 |
|-------------|-------------|-----------|
| `模型檔案` | 生成的 3D 模型檔案，僅為向下相容而保留。 | STRING |
