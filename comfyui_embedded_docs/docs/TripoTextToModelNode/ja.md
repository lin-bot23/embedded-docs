...
| `auto_size` | テクスチャ付きモデルを実世界のサイズ（メートル単位）にスケーリングします。Tripo はサイズをモデルのシーントランスフォームとして保存し、モデルが変換、リギング、またはリターゲティングされる際にベイクします。テクスチャがない場合は無視されます。（デフォルト: True） | BOOLEAN | No | true / false |
