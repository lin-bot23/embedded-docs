...
분할된 3D 모델의 파트를 완성하고 메시의 누락되거나 손상된 영역을 복구합니다. Tripo 메시 분할 결과의 작업 ID를 받아 Tripo에 완성 작업을 요청하고, 작업이 완료될 때까지 기다립니다. 선택적으로 특정 파트 이름으로 작업 범위를 제한할 수 있습니다. 완성된 모델은 GLB 파일로 반환됩니다.
...
| `part_names` | 완성할 파트 이름을 쉼표로 구분하여 입력합니다. 비워 두면 모든 파트를 완성합니다. 기본값: 빈 문자열. 이름 주변의 추가 공백은 제거되며 중복된 이름은 무시됩니다. | STRING | 아니요 | 자유 텍스트 또는 비어 있음 |
...
| `모델 task_id` | 완료된 Tripo 메시 완성 작업의 작업 ID입니다. 모델 작업 ID를 기대하는 다른 Tripo 노드의 입력으로 사용할 수 있습니다. | MODEL_TASK_ID |
