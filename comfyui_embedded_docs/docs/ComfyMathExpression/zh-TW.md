...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `expression` | 要評估的數學公式，以文字編寫（例如 `a + b`），使用輸入值的字母名稱作為變數。多行輸入。（預設值："a + b"） | STRING | 是 | N/A |
| `values` | 可擴充的輸入值群組，為公式提供變數。群組中新增的每個值會自動依序取得下一個小寫字母名稱，從 `a` 開始（`a`、`b`、`c`……），然後該名稱便可在 `expression` 中使用。每個項目接受數字（INT 或 FLOAT）或布林值（TRUE/FALSE）。 | FLOAT, INT, BOOLEAN | 是 | 1 至 26 values, named `a` to `z` |
