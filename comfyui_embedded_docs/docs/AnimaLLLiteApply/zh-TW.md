...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `模型` | 要套用修補程式的基本擴散模型 | MODEL | 是 | |
| `模型補丁` | 要套用的預先配置動畫修補程式 | MODEL_PATCH | 是 | |
| `影像` | 用於引導生成的參考影像。僅使用前 3 個色彩通道 (RGB) | IMAGE | 是 | |
| `強度` | 修補程式的效果強度（預設值：1.0） | FLOAT | 是 | -10.0 至 10.0 |
| `起始百分比` | 修補程式開始生效時的去噪流程百分比（預設值：0.0） | FLOAT | 是 | 0.0 至 1.0 |
| `結束百分比` | 修補程式停止生效時的去噪流程百分比（預設值：1.0） | FLOAT | 是 | 0.0 至 1.0 |
| `遮罩` | 選用遮罩，用於將修補程式效果限制在影像的特定區域 | MASK | 否 | |
