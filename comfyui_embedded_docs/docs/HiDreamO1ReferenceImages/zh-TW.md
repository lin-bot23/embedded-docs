...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `positive` | 要附加參考圖片的正向條件。 | CONDITIONING | 是 | - |
| `negative` | 要附加參考圖片的負向條件。 | CONDITIONING | 是 | - |
| `images` | 參考圖片依數字插槽順序使用。提供圖片時，圖片會附加至正向與負向條件。 | IMAGE | 否 | 0 到 100 張圖片（`image_1` 至 `image_100`） |
...
| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| `positive` | 已附加參考圖片的正向條件。 | CONDITIONING |
| `negative` | 已附加參考圖片的負向條件。 | CONDITIONING |
