...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `model_task_id` | 要添加紋理的 Tripo 模型任務 ID。接受模型任務 ID 與分割任務 ID。 | MODEL_TASK_ID | 是 | - |
| `texture` | 已忽略：此節點一律會生成紋理。為舊版工作流程保留。（預設值：True） | BOOLEAN | 否 | true<br>false |
| `pbr` | PBR 材質貼圖（基底色、金屬度、粗糙度、法線）；關閉時會產生純色紋理。（預設值：True） | BOOLEAN | 否 | true<br>false |
| `texture_seed` | 紋理生成的隨機種子。使用相同種子與相同輸入會產生相同結果。（預設值：42） | INT | 否 | 0 – 2147483647 |
| `texture_quality` | 紋理解析度品質：detailed = HD 紋理，extreme = 8K Ultra 紋理。（預設值："standard"）。約略費用：standard $0.10、detailed $0.20、extreme $0.30。 | COMBO | 否 | "standard"<br>"detailed"<br>"extreme" |
| `texture_alignment` | 用於將生成的紋理對齊至模型的方法。（預設值："original_image"）。 | COMBO | 否 | "original_image"<br>"geometry" |
...
| `reference` | 引導紋理生成的參考圖片。無法與 `texture_prompt` 或 `style_image` 同時使用。（預設值："none"） | DYNAMIC_COMBO | 否 | "none"<br>"image"<br>"multiview" |
...
| 輸出名稱 | 描述 | 資料類型 |
|-------------|-------------|-----------|
| `model_file` | 生成的模型檔案（僅供向後相容）。 | STRING |
| `model task_id` | 已完成紋理生成任務的任務 ID，可作為其他 Tripo 節點的輸入。 | MODEL_TASK_ID |
