...
|-----------|-------------|-----------|----------|-------|
| `model_task_id` | L’ID de tâche Tripo du modèle à texturer. Accepte les ID de tâche de modèle et les ID de tâche de segmentation. | MODEL_TASK_ID | Oui | - |
...
| `texture_seed` | Graine aléatoire (seed) pour la génération de textures. Utiliser la même graine avec les mêmes entrées produit le même résultat. (par défaut : 42) | INT | Non | 0 – 2147483647 |
...
|-------------|-------------|-----------|
| `model_file` | Le fichier de modèle généré (uniquement pour la rétrocompatibilité). | STRING |
| `model task_id` | L’ID de la tâche de génération de textures terminée, utilisable comme entrée pour d’autres nœuds Tripo. | MODEL_TASK_ID |
