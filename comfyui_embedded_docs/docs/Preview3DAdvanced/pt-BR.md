...
|-----------|-------------|-----------|----------|-------|
| `modelo_3d` | Arquivo do modelo 3D proveniente de um nó 3D upstream. | FILE3D | Sim | GLB, GLTF, FBX, OBJ, STL, USDZ ou qualquer formato 3D suportado |
| `info_modelo_3d` | Metadados de informações do modelo opcional. Opção avançada. | LOAD3DMODELINFO | Não | - |
| `estado_da_janela_de_visualização` | O estado atual da vista, contendo informações da câmera e do modelo. | LOAD3D | Sim | - |
| `info_câmera` | Configuração da câmera opcional para a vista 3D. Opção avançada. | LOAD3DCAMERA | Não | - |
| `largura` | Largura da prévia em pixels. Padrão: 1024. | INT | Sim | 1 a 4096 |
| `altura` | Altura da prévia em pixels. Padrão: 1024. | INT | Sim | 1 a 4096 |
...
|-------------|-------------|-----------|
| `arquivo_modelo` | Arquivo do modelo 3D passado pelo entrada. | FILE3D |
| `info_câmera` | Metadados de informações do modelo, seja do entrada ou do estado da vista. | LOAD3DMODELINFO |
| `info_modelo_3d` | Configuração da câmera, seja do entrada ou do estado da vista. | LOAD3DCAMERA |
| `largura` | Largura da prévia em pixels. | INT |
| `altura` | Altura da prévia em pixels. | INT |
