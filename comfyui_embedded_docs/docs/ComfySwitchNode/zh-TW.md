...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `switch` | 決定要傳遞哪一個輸入的布林條件。啟用（true）時，會選取 `on_true` 輸入；停用（false）時，則會選取 `on_false` 輸入。 | BOOLEAN | 是 |  |
| `on_false` | 當 `switch` 停用（false）時，要傳遞到輸出的資料。只有在 `switch` 為 false 時才需要此輸入。 | MATCH_TYPE | 否 |  |
| `on_true` | 當 `switch` 啟用（true）時，要傳遞到輸出的資料。只有在 `switch` 為 true 時才需要此輸入。 | MATCH_TYPE | 否 |  |
...
| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| `output` | 被選取的資料。當 `switch` 為 true 時，此為 `on_true` 輸入的值；當 `switch` 為 false 時，此為 `on_false` 輸入的值。 | MATCH_TYPE |
