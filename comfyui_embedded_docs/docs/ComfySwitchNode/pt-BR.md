...
| --- | --- | --- | --- | --- |
| `switch` | Uma condição booleana que determina qual entrada passar. Quando ativado (true), a entrada `on_true` é selecionada. Quando desativado (false), a entrada `on_false` é selecionada. | BOOLEAN | Sim | |
| `on_false` | Os dados a serem passados para a saída quando o `switch` estiver desativado (false). Esta entrada é necessária apenas quando o `switch` é false. | MATCH_TYPE | Não | |
| `on_true` | Os dados a serem passados para a saída quando o `switch` estiver ativado (true). Esta entrada é necessária apenas quando o `switch` é true. | MATCH_TYPE | Não | |
...
| --- | --- | --- |
| `output` | Os dados selecionados. Este é o valor da entrada `on_true` quando o `switch` é true ou o valor da entrada `on_false` quando o `switch` é false. | MATCH_TYPE |
