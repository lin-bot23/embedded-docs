...
|-----------|-------------|-----------|----------|-------|
| `model` | 用於產生回應的 Gemini 模型。 | DYNAMIC_COMBO | Yes | `"Gemini 3.8 Flash"`<br>`"Gemini 3.7 Flash"`<br>`"Gemini 3.5 Flash"`<br>`"Gemini 3.1 Pro"`<br>`"Gemini 3.1 Flash-Lite"` |
| `prompt` | 輸入至模型的文字。請包含詳細的指示、問題或上下文。必須包含至少一個非空白字元。（預設值：""） | STRING | Yes |  |
| `seed` | 取樣用的種子。設為 0 即使用隨機種子。不保證輸出具有確定性。（預設值：42） | INT | Yes | 0 至 2147483647 |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `thinking_level` | 模型在回答前進行內部推理的強度。HIGH 可提升困難任務的品質，但會消耗更多（思考）token 且速度較慢。（預設值："MEDIUM"） | COMBO | Yes | `"LOW"`<br>`"MEDIUM"`<br>`"HIGH"` |
| `max_output_tokens` | 要產生的最大 token 數，包含模型內部的思考。當 thinking_level 為 HIGH 時，數值過低可能導致沒有空間產生答案；若回應為空或被截斷，請調高此值。模型完成後會提前停止，因此對於簡短回覆而言，較高的上限不會增加額外成本。（預設值：32768） | INT | Yes | 16 至 65536 |
...
|-----------|-------------|-----------|----------|-------|
| `thinking_level` | 模型在回答前進行內部推理的強度。HIGH 可提升困難任務的品質，但會消耗更多（思考）token 且速度較慢。（預設值："MEDIUM"） | COMBO | Yes | `"LOW"`<br>`"MEDIUM"`<br>`"HIGH"` |
| `temperature` | 控制隨機性。數值越低越聚焦／越具確定性，數值越高越有創意。（預設值：1.0） | FLOAT | Yes | 0.0 至 2.0 |
| `top_p` | 核取樣（nucleus sampling）：從累積機率達到 top_p 的最小 token 集合中取樣。（預設值：0.95） | FLOAT | Yes | 0.0 至 1.0 |
| `max_output_tokens` | 要產生的最大 token 數，包含模型內部的思考。當 thinking_level 為 HIGH 時，數值過低可能導致沒有空間產生答案；若回應為空或被截斷，請調高此值。模型完成後會提前停止，因此對於簡短回覆而言，較高的上限不會增加額外成本。（預設值：32768） | INT | Yes | 16 至 65536 |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `thinking_level` | 模型在回答前進行內部推理的強度。HIGH 可提升困難任務的品質，但會消耗更多（思考）token 且速度較慢。（預設值："MEDIUM"） | COMBO | Yes | `"MINIMAL"`<br>`"LOW"`<br>`"MEDIUM"`<br>`"HIGH"` |
| `temperature` | 控制隨機性。數值越低越聚焦／越具確定性，數值越高越有創意。（預設值：1.0） | FLOAT | Yes | 0.0 至 2.0 |
| `top_p` | 核取樣（nucleus sampling）：從累積機率達到 top_p 的最小 token 集合中取樣。（預設值：0.95） | FLOAT | Yes | 0.0 至 1.0 |
| `max_output_tokens` | 要產生的最大 token 數，包含模型內部的思考。當 thinking_level 為 HIGH 時，數值過低可能導致沒有空間產生答案；若回應為空或被截斷，請調高此值。模型完成後會提前停止，因此對於簡短回覆而言，較高的上限不會增加額外成本。（預設值：32768） | INT | Yes | 16 至 65536 |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `thinking_level` | 模型在回答前進行內部推理的強度。HIGH 可提升困難任務的品質，但會消耗更多（思考）token 且速度較慢。（預設值："HIGH"） | COMBO | Yes | `"LOW"`<br>`"HIGH"` |
| `temperature` | 控制隨機性。數值越低越聚焦／越具確定性，數值越高越有創意。（預設值：1.0） | FLOAT | Yes | 0.0 至 2.0 |
| `top_p` | 核取樣（nucleus sampling）：從累積機率達到 top_p 的最小 token 集合中取樣。（預設值：0.95） | FLOAT | Yes | 0.0 至 1.0 |
| `max_output_tokens` | 要產生的最大 token 數，包含模型內部的思考。當 thinking_level 為 HIGH 時，數值過低可能導致沒有空間產生答案；若回應為空或被截斷，請調高此值。模型完成後會提前停止，因此對於簡短回覆而言，較高的上限不會增加額外成本。（預設值：32768） | INT | Yes | 16 至 65536 |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `thinking_level` | 模型在回答前進行內部推理的強度。HIGH 可提升困難任務的品質，但會消耗更多（思考）token 且速度較慢。（預設值："LOW"） | COMBO | Yes | `"LOW"`<br>`"HIGH"` |
| `temperature` | 控制隨機性。數值越低越聚焦／越具確定性，數值越高越有創意。（預設值：1.0） | FLOAT | Yes | 0.0 至 2.0 |
| `top_p` | 核取樣（nucleus sampling）：從累積機率達到 top_p 的最小 token 集合中取樣。（預設值：0.95） | FLOAT | Yes | 0.0 至 1.0 |
| `max_output_tokens` | 要產生的最大 token 數，包含模型內部的思考。當 thinking_level 為 HIGH 時，數值過低可能導致沒有空間產生答案；若回應為空或被截斷，請調高此值。模型完成後會提前停止，因此對於簡短回覆而言，較高的上限不會增加額外成本。（預設值：32768） | INT | Yes | 16 至 65536 |
