...
Tripo: Retopology は、以前の Tripo ノードで生成されたハイポリゴンの 3D モデルを受け取り、クリーンなトポロジーを持つローポリゴン版として再構築します。モデルを Tripo リトポロジーサービスに送信し、タスクの完了を待ってから、完成したモデルをダウンロードし、他の Tripo ノードで使用できるようにそのタスク ID を公開します。
...
注: `face_limit` が -1 に設定されている場合、Tripo が面数を自動的に決定します。`quad` が有効な場合、最大面数は 20,000 三角形ではなく 10,000 四角形となり、結果は FBX として提供されます（GLB 出力は空のままです）。`part_names` が空の場合、モデル全体が処理されます。`face_limit` が -1 以外で、許可された範囲外の場合は、ノードがエラーを発生させます。
...
| `model task_id` | 完了したリトポロジー結果のタスク ID。他の Tripo ノードに渡してこのモデルを参照できます。 | STRING |
| `GLB` | リトポロジー後のローポリゴンモデル（GLB 形式）。`quad` が有効な場合は空です。 | GLB FILE |
| `FBX` | リトポロジー後のローポリゴンモデル（FBX 形式）。`quad` が有効な場合にのみ格納されます。 | FBX FILE |
