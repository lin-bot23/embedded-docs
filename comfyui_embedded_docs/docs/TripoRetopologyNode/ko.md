...
Tripo: Retopology는 이전 Tripo 노드에서 생성된 고폴리 3D 모델을 가져와 깨끗한 토폴로지를 갖춘 저폴리 버전으로 재구성합니다. 이 모델을 Tripo 리토폴로지 서비스에 제출하고, 작업이 완료될 때까지 기다린 다음, 완성된 모델을 다운로드하고 다른 Tripo 노드에서 사용할 수 있도록 해당 작업 ID를 노출합니다.
...
참고: `face_limit`이 -1로 설정되면 Tripo가 면 수를 자동으로 결정합니다. `quad`가 활성화되면 최대 면 제한은 20,000개의 삼각형 대신 10,000개의 쿼드이며, 결과는 FBX로 제공됩니다(GLB 출력은 비어 있습니다). `part_names`가 비어 있으면 전체 모델이 처리됩니다. `face_limit`이 -1이 아니면서 허용 범위를 벗어나면 노드에서 오류를 발생시킵니다.
...
| `GLB` | GLB 형식의 리토폴로지된 저폴리 모델입니다. `quad`가 활성화되면 비어 있습니다. | GLB FILE |
| `FBX` | FBX 형식의 리토폴로지된 저폴리 모델입니다. `quad`가 활성화된 경우에만 채워집니다. | FBX FILE |
