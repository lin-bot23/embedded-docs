...
Note: When `face_limit` is set to -1, Tripo decides the face count automatically. When `quad` is enabled, the maximum face limit is 10,000 quads instead of 20,000 triangles, and the result is provided as FBX (the GLB output stays empty). When `part_names` is empty, the whole model is processed. If `face_limit` is anything other than -1 and falls outside the allowed range, the node raises an error.
