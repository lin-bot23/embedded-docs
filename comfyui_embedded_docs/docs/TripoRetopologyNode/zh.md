...
Tripo: Retopology 接收由之前的 Tripo 节点生成的高多边形 3D 模型，并将其重建为具有干净拓扑的低多边形版本。它会将模型提交至 Tripo 重拓扑服务，等待任务完成，然后下载完成的模型，并提供其任务 ID 以供其他 Tripo 节点使用。
...
注意：当 `face_limit` 设置为 -1 时，Tripo 会自动决定面数。启用 `quad` 时，最大面数上限为 10,000 个四边形，而不是 20,000 个三角形，并且结果以 FBX 格式提供（GLB 输出保持为空）。当 `part_names` 为空时，将处理整个模型。如果 `face_limit` 不是 -1，且超出允许范围，节点会报错。
...
| `模型 task_id` | 已完成重拓扑结果的任务 ID。可传递给其他 Tripo 节点以引用此模型。 | STRING |
