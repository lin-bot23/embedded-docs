...
|-----------|-------------|-----------------|--------|-------|
| `modèle` | Le modèle utilisé pour prétraiter les plongements textuels et exécuter la tête de durée. | MODEL | Oui | N/A |
| `positif` | Le conditionnement qui fournit les plongements textuels du prompt et les métadonnées pour la prédiction de durée. | CONDITIONING | Oui | N/A |
| `duration_head` | Tête de durée LTX 2.4 chargée avec ModelPatchLoader. Doit être une tête de durée LTX. | MODEL_PATCH | Oui | N/A |
| `taux d’images` | Taux d’images par seconde utilisé pour convertir les secondes en images (défaut : 24.0). | FLOAT | Oui | 1.0 à 120.0 |
...
|---------------|-------------|-----------------|
| `nombre_de_frames` | La durée prédite convertie en nombre d’images et ajustée sur la grille d’images 8k+1 du VAE. | INT |
| `secondes` | Durée prédite brute (non bornée). Cette valeur est celle d’avant l’ajustement sur la grille d’images. | FLOAT |
