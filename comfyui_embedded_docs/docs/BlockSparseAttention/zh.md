...
| 参数 | 描述 | 数据类型 | 必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `model` | 要应用块稀疏注意力的 ComfyUI 模型。 | 模型 | 是 | N/A |
...
| `dense_blocks` | 表示始终运行密集注意力的 Transformer 块的字符串。 | 字符串 | 否 | 默认："" |
| `min_tokens` | 模型使用密集注意力的序列中令牌的最小数量。 | 整数 | 否 | 默认：12288，最小：0，最大：1 << 20，步长：512 |
| `extra_tokens` | 每个查询块在其所选块之外关注的额外最高得分令牌的数量。 | 整数 | 否 | 默认：256，最小：0，最大：256，步长：64 |
