...
|-----------|-------------|-----------|----------|-------|
| `model` | Le modèle ComfyUI auquel appliquer l'attention sparse par bloc. | MODÈLE | Oui | N/A |
...
| `tau` | Le seuil en sigmas de distribution des scores pour la méthode sol-attn. | FLOTTANT | Non | par défaut : 1.3, min : 0.0, max : 4.0, pas : 0.05 |
| `keep_percent` | Le pourcentage de blocs de clé que chaque bloc de requête conserve exactement pour la méthode sla. | FLOTTANT | Non | par défaut : 10.0, min : 0.5, max : 95.0, pas : 0.5 |
| `start_percent` | Le pourcentage de point où l'attention sparse commence. | FLOTTANT | Non | par défaut : 0.2, min : 0.0, max : 1.0, pas : 0.01 |
| `end_percent` | Le pourcentage de point où l'attention sparse se termine. | FLOTTANT | Non | par défaut : 1.0, min : 0.0, max : 1.0, pas : 0.01 |
| `dense_blocks` | Une chaîne représentant les blocs de transformer qui doivent toujours utiliser l'attention dense. | CHAÎNE | Non | par défaut : "" |
...
|-------------|-------------|-----------|
| `model` | Le modèle ComfyUI auquel l'attention sparse par bloc a été appliquée. | MODÈLE |
...
- Le paramètre `min_tokens` doit être un entier positif.
