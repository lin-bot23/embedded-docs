...
|-----------|-------------|---------------|-----------|-------|
| `original_model_task_id` | ID de tarea del modelo de Tripo que se va a convertir. Debe provenir de una tarea anterior de Tripo, ya sea de generación de modelos, rigging, retargeting o segmentación. Si el ID falta o está vacío, el nodo genera un error. | STRING (ID de tarea de Tripo) | Sí | MODEL_TASK_ID<br>RIG_TASK_ID<br>RETARGET_TASK_ID<br>SEGMENT_TASK_ID |
| `format` | Formato de archivo de destino para el modelo 3D convertido. | COMBO | Sí | GLTF<br>USDZ<br>FBX<br>OBJ<br>STL<br>3MF |
| `quad` | Convierte triángulos en cuadriláteros cuando está habilitado (predeterminado: False). | BOOLEAN | No | True o False |
| `face_limit` | Número máximo de caras en el modelo convertido. Establece -1 para indicar que no hay límite (predeterminado: -1). | INT | No | -1 a 2000000 |
| `texture_size` | Resolución de las texturas de salida en píxeles (predeterminado: 4096). | INT | No | 128 a 8192 |
| `texture_format` | Formato de archivo utilizado para las texturas exportadas (predeterminado: JPEG). | COMBO | No | BMP<br>DPX<br>HDR<br>JPEG<br>OPEN_EXR<br>PNG<br>TARGA<br>TIFF<br>WEBP |
