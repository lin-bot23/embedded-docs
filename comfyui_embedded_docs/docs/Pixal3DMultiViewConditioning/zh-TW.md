...
| `fov` | 视角框選中的水平視野角（度）。 | FLOAT | 是 | 1.0 - 170.0 |
| `front` | 物體前側的方形視圖，帶有 alpha 通道或黑色背景。 | IMAGE | 是 | N/A |
| `left` | 物體左側的方形視圖，帶有 alpha 通道或黑色背景。 | IMAGE | 選擇性 | N/A |
| `back` | 物體後側的方形視圖，帶有 alpha 通道或黑色背景。 | IMAGE | 選擇性 | N/A |
| `right` | 物體右側的方形視圖，帶有 alpha 通道或黑色背景。 | IMAGE | 選擇性 | N/A |

## 輸出

| 輸出名 | 描述 | 資料類型 |
|-------------|-------------|-----------|
| `positive` | Pixal3D 多視角條件設定節點的正向條件輸出。 | CONDITIONING |
| `negative` | Pixal3D 多視角條件設定節點的負向條件輸出。 | CONDITIONING |
