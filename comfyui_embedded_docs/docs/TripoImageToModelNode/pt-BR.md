...
|-----------|-------------|-----------|----------|-------|
| `image` | A imagem de entrada usada para gerar o modelo 3D. Uma imagem deve ser fornecida; caso contrário, o nó gera um erro. | IMAGEM | Sim | - |
| `model_version` | A versão do modelo a ser usada para a geração. | COMBO | Não | `"v1.4"`<br>`"v3.0"`<br>`"v3.5"`<br>`"v3.6"` |
| `style` | Não é mais suportado pela Tripo e ignorado. Mantido para fluxos de trabalho antigos. (padrão: `"None"`) | COMBO | Não | `"None"`<br>`"realistic"`<br>`"cartoon"`<br>`"sculpture"`<br>`"low_poly"` |
| `texture` | Gera mapas de textura. Desligado retorna geometria nua e ignora `pbr`. (padrão: True) | BOOLEAN | Não | True<br>False |
| `pbr` | Mapas de material PBR (cor de base, metálico, rugosidade, normal). Requer `texture`. (padrão: True) | BOOLEAN | Não | True<br>False |
| `model_seed` | Semente aleatória para a geração do modelo. (padrão: 42) | INT | Não | 0 a 2147483647 |
| `orientation` | Configuração de orientação para o modelo gerado. (padrão: `"default"`) | COMBO | Não | `"default"`<br>`"front"`<br>`"back"`<br>`"left"`<br>`"right"`<br>`"top"`<br>`"bottom"` |
| `texture_seed` | Semente aleatória para a geração de textura. (padrão: 42) | INT | Não | 0 a 2147483647 |
| `texture_quality` | Nível de qualidade para a geração de textura: `detailed` = texturas HD, `extreme` = texturas Ultra 8K. (padrão: `"standard"`) | COMBO | Não | `"standard"`<br>`"detailed"`<br>`"extreme"` |
| `texture_alignment` | Método de alinhamento para a mapeamento de textura. (padrão: `"original_image"`) | COMBO | Não | `"original_image"`<br>`"geometry"` |
| `face_limit` | Número máximo de faces. -1 permite que a Tripo escolha adaptivamente (aproximadamente 1,4M faces na versão 3.x padrão, 2M na detalhada). A Tripo limita silenciosamente: v2.5 em 500.000, malhas quadrangulares em 150.000. (padrão: -1) | INT | Não | -1 a 2000000 |
...
| `geometry_quality` | Nível de qualidade para a geração de geometria. (padrão: `"standard"`) | COMBO | Não | `"standard"`<br>`"detailed"` |
...
| `model task_id` | O ID da tarefa para rastrear o processo de geração do modelo. | ID_DA_TAREFA_DO_MODELO |
