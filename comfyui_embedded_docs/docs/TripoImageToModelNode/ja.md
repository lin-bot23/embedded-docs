...
| `geometry_quality` | ジオメトリ生成の品質レベルです。（デフォルト: `"standard"`） | COMBO | いいえ | `"standard"`<br>`"detailed"` |
...
| 出力名 | 説明 | データ型 |
|-------------|-------------|-----------|
| `model_file` | 生成された3Dモデルファイルです（後方互換性のためだけに用意されています）。 | STRING |
| `model task_id` | モデル生成プロセスを追跡するためのタスクIDです。 | MODEL_TASK_ID |
