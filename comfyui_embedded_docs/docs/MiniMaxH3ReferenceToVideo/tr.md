...
| `width` | Üretilen videonun piksel cinsinden genişliği (varsayılan: 1344). | INT | Evet | 32 ila 16384 (adım 32) |
| `height` | Üretilen videonun piksel cinsinden yüksekliği (varsayılan: 768). | INT | Evet | 32 ila 16384 (adım 32) |
| `length` | 24 fps'de kare sayısı; 124 = ~5 sn, eğitilmiş aralık ~124-362'dir (varsayılan: 124). | INT | Evet | 5 ila 3600 (adım 17) |
| `ref_image_size` | Referans görsel boyutlandırması. `match`, her referans görselini en-boy oranını koruyarak yalnızca küçültüp üretimin piksel alanına ölçekler; `max`, en iyi kimlik doğruluğu için referans hattının 2048px kısa kenarını kullanır. Referans tokenları her örnekleme adımında taşındığından `max` birkaç kat daha yavaş olabilir (varsayılan: `match`). | COMBO | Evet | `"match"`<br>`"max"` |
| `ref_images` | Genişletilebilir yuva: en fazla 9 referans görseli bağlayın (`ref_image_1` ... `ref_image_9`). Referans görseller, daha büyüklerse 2048px kısa kenara küçültülür ve asla büyütülmez. | IMAGE | Hayır | 0 ila 9 |
| `ref_videos` | Genişletilebilir yuva: en fazla 3 referans videosu bağlayın (`ref_video_1` ... `ref_video_3`). 24 fps'de (2-15 sn) referans video kareleri. | IMAGE | Hayır | 0 ila 3 |
| `ref_video_audios` | Genişletilebilir yuva: en fazla 3 film müziği bağlayın (`ref_video_audio_1` ... `ref_video_audio_3`). Aynı numaralı referans videosunun film müziği. | AUDIO | Hayır | 0 ila 3 |
| `ref_audios` | Genişletilebilir yuva: en fazla 3 bağımsız referans ses klibi bağlayın (`ref_audio_1` ... `ref_audio_3`). | AUDIO | Hayır | 0 ila 3 |
...
|-------------|-------------|-----------|
| `positive` | Kodlanmış promptu içeren koşullandırma. Referans medya ve ilgili VAE'ler sağlandığında, MiniMax H3 modeli tarafından kullanılan kodlanmış referans görsel, video ve ses içeriğini de içerir. | CONDITIONING |
