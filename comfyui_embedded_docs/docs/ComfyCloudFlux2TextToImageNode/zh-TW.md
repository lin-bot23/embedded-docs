...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
|-----------|-------------|-----------|------|-------|
| `提示詞` | 描述要生成之圖片的文字提示。提交前會移除前後空白字元。 | STRING | 是 | 1 至 4096 個字元 |
| `種子` | 用於控制生成結果以具備可重現性的隨機種子（預設值：42）。 | INT | 是 | 0 至 18446744073709551615 |
| `長寬比` | 輸出圖片的長寬比（預設值："1:1"）。 | COMBO | 是 | "1:1"<br>"3:4"<br>"2:3"<br>"3:2"<br>"4:3"<br>"16:9"<br>"9:16"<br>"21:9" |
| `百萬像素` | 總像素預算。在方形比例下，1.0 約為 1024x1024（預設值：1.0）。 | FLOAT | 是 | 0.1 至 16.0（步進 0.1） |
