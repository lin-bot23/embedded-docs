...
ImageColorSpace 노드는 이미지를 sRGB(Rec.709), 선형 Rec.709, HDR(Rec.2020 HLG), HDR PQ(Rec.2020 PQ) 색 공간 간에 변환합니다. SDR 출력으로 변환하거나 HDR PQ에서 HDR로 변환할 때는 배치 전체에 걸쳐 초과 휘도를 톤 매핑하고 색역을 벗어난 색상을 압축합니다. 선형 출력 및 선형에서 HDR로의 변환은 톤 매핑 없이 확장 값을 유지합니다. 변환은 float32로 계산되며, 알파 채널은 변경 없이 그대로 전달됩니다.
