...
ImageColorSpace 节点可在 sRGB（Rec.709）、线性 Rec.709、HDR（Rec.2020 HLG）和 HDR PQ（Rec.2020 PQ）色彩空间之间转换图像。当缩窄色彩空间时，它会在整个批次中对超出亮度进行色调映射，并压缩超出色域的颜色。转换以 float32 计算，任何 alpha 通道都会原样传递。
