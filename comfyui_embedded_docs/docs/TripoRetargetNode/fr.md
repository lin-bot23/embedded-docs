...
|-----------|-------------|----------------|--------|-------|
| `ID_tâche_modèle_original` | L’identifiant de tâche du modèle 3D préalablement riggé à retargeter. La tâche référencée doit être une tâche de rig ; un rig réalisé avec la spécification Mixamo sur la version de modèle v1.0 ne peut pas être utilisé pour le retarget. | RIG_TASK_ID | Oui | - |
